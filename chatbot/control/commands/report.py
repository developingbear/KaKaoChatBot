...
class Report(Command):
    ...
    def makeReportMsg():
        ...
        absenceList = DBHandler.getTodayWorkplaceList('DS사업1)e커머스팀', '휴직')
        newline = '\n'
        return f'''
📋 오쇼핑IT서비스팀
{date.today()}
...
{newline + '▪ 재택 : ' + homeworkingList if homeworkingList else ""}{newline + '▪ 휴가 : ' + vacationList if vacationList else ""}{newline + '▪ 외근 : ' + workoutsideList if workoutsideList else ""}{newline + '▪ 탄력 : ' + flexibleworkList if flexibleworkList else ""}{newline + '▪ 휴직 : ' + absenceList if absenceList else ""}
